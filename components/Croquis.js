...
const predefinedUnits = [
    { type: 'engine', label: 'Autobomba', icon: React.createElement(EngineIcon, { className: "w-5 h-5" }), color: 'bg-red-600 hover:bg-red-500', defaultLabel: 'E-1' },
    { type: 'ladder', label: 'Hidroelevador', icon: React.createElement(LadderIcon, { className: "w-5 h-5" }), color: 'bg-blue-600 hover:bg-blue-500', defaultLabel: 'H-1' },
    { type: 'ambulance', label: 'Ambulancia', icon: React.createElement(AmbulanceIcon, { className: "w-5 h-5" }), color: 'bg-green-600 hover:bg-green-500', defaultLabel: 'A-1' },
    { type: 'command', label: 'Puesto Comando', icon: React.createElement(CommandPostIcon, { className: "w-5 h-5" }), color: 'bg-orange-600 hover:bg-orange-500', defaultLabel: 'PC-1' },
    { type: 'person', label: 'Personal', icon: React.createElement(PersonIcon, { className: "w-5 h-5" }), color: 'bg-purple-600 hover:bg-purple-500', defaultLabel: 'P-1' }
];

const Croquis = ({ isActive, onSketchCapture, onUnlockSketch, initialLayer = 'street' }) => {
    const containerRef = useRef(null);
    const mapContainerRef = useRef(null);
    const mapRef = useRef(null);
    const isInitialized = useRef(false);
    const ecologicalReserveOverlayRef = useRef(null);
    ...
    const loadStateFromLocalStorage = useCallback(() => {
        ...
        try {
            ...
            const loadedPoints = (savedState.points || []).map((p) => {
            ...
            const loadedZones = (savedState.zones || []).map((z) => {
            ...
            const loadedUnits = (savedState.units || []).map((u) => {
            ...
            const loadedTexts = (savedState.texts || []).map((t) => {
            ...
            const loadedLines = (savedState.lines || []).map((l) => {
    ...
    return (
        React.createElement("div", { ref: containerRef, className: `flex flex-col gap-4 transition-all duration-300 ${isMaximized ? 'fixed inset-0 bg-zinc-900 z-50 p-4' : 'h-[85vh]'}` },
            mode === 'drawing' ? (
                 React.createElement("div", { className: "bg-zinc-800/60 p-3 rounded-xl flex flex-wrap items-center justify-between gap-2" },
                     React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        ...
                        React.createElement(ToolButton, { toolName: "impact", label: "Zona Impacto", icon: React.createElement("div", { className: "w-6 h-6 rounded-full bg-red-500 border-2 border-white/50" }) }),
                        React.createElement(ToolButton, { toolName: "adjacency", label: "Zona Adyac.", icon: React.createElement("div", { className: "w-6 h-6 rounded-full bg-yellow-500 border-2 border-white/50" }) }),
                        React.createElement(ToolButton, { toolName: "influence", label: "Zona Influ.", icon: React.createElement("div", { className: "w-6 h-6 rounded-full bg-green-500 border-2 border-white/50" }) }),
                    ...
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        ...
                        React.createElement("button", { onClick: handleDownloadImage, className: "flex items-center gap-2 px-3 py-2 bg-purple-600 hover:bg-purple-500 rounded-md text-white font-semibold" },
                            React.createElement(PhotoIcon, { className: "w-5 h-5" }), "Descargar Imagen"
                        ),
            ...
            React.createElement("div", { className: "relative w-full h-full" },
                ...
                React.createElement("div", { className: "absolute top-2 right-2 z-[1000] flex flex-col gap-2" },
                     React.createElement("button", { onClick: () => setIsMaximized(!isMaximized), className: "p-2 bg-zinc-800/80 hover:bg-zinc-700/80 rounded-md text-white" },
                        isMaximized ? React.createElement(MinimizeIcon, { className: "w-5 h-5" }) : React.createElement(MaximizeIcon, { className: "w-5 h-5" })
                    ),
                    React.createElement("input", { ref: fileInputRef, type: "file", accept: ".kml", onChange: handleFileUpload, style: { display: 'none' } }),
                    React.createElement("button", { onClick: () => fileInputRef.current?.click(), title: "Importar KML", className: "p-2 bg-zinc-800/80 hover:bg-zinc-700/80 rounded-md text-white" },
                        React.createElement(FolderPlusIcon, { className: "w-5 h-5" })
                    )
                ),
                mode === 'drawing' && tool && tool !== 'unit' && tool !== 'text' && (
                    React.createElement("div", { className: "absolute top-2 left-1/2 -translate-x-1/2 z-[1000] bg-zinc-800/80 p-3 rounded-lg text-white text-sm font-semibold animate-fade-in" },
                        `Haga clic en el mapa para ${tool === 'point' ? 'marcar un punto' : tool === 'line' ? 'empezar a dibujar' : 'colocar el centro de la zona'}.`
                    )
                ),
                mode === 'drawing' && (
                    React.createElement("div", { className: "absolute top-16 left-2 z-[1000] flex flex-col gap-2 w-72" },
                        (tool === 'impact' || tool === 'adjacency' || tool === 'influence') && (
                             React.createElement("div", { className: "bg-zinc-900/50 p-3 rounded-lg flex flex-col gap-2 animate-fade-in" },
                                React.createElement("label", { className: "text-sm text-white font-medium", htmlFor: "radius-impact" }, "Radio Zona Impacto (m)"),
                                React.createElement("input", { type: "range", id: "radius-impact", min: "10", max: "500", value: impactRadius, onChange: e => setImpactRadius(Number(e.target.value)), className: "w-full" }),
                                React.createElement("label", { className: "text-sm text-white font-medium", htmlFor: "radius-adjacency" }, "Radio Zona Adyacencia (m)"),
                                React.createElement("input", { type: "range", id: "radius-adjacency", min: "10", max: "1000", value: adjacencyRadius, onChange: e => setAdjacencyRadius(Number(e.target.value)), className: "w-full" }),
                                 React.createElement("label", { className: "text-sm text-white font-medium", htmlFor: "radius-influence" }, "Radio Zona Influencia (m)"),
                                React.createElement("input", { type: "range", id: "radius-influence", min: "10", max: "2000", value: influenceRadius, onChange: e => setInfluenceRadius(Number(e.target.value)), className: "w-full" })
                            )
                        ),

                        tool === 'unit' && (
                            React.createElement("div", { className: "bg-zinc-900/50 p-3 rounded-lg flex flex-col gap-3 animate-fade-in" },
                                React.createElement("h4", { className: "text-sm font-semibold text-white" }, "Añadir Unidades"),
                                React.createElement("p", { className: "text-xs text-zinc-400 -mt-2" }, "Seleccione una unidad y luego haga clic en el mapa."),
                                ...
                                React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-3 gap-2" },
                                    predefinedUnits.map((unit) => (
                                        React.createElement("button", {
                                            ...
                                            unit.icon,
                                            React.createElement("span", null, unit.label)
                                        )
                                    ))
                                ),

                                React.createElement("div", { className: "border-t border-zinc-700 pt-3 mt-1" },
                                    React.createElement("h5", { className: "text-xs font-semibold text-zinc-300 mb-1" }, "Unidad Personalizada"),
                                    React.createElement("div", { className: "flex gap-2" },
                                        React.createElement("select", { value: unitType, onChange: (e) => setUnitType(e.target.value), className: "bg-zinc-700 border-zinc-600 rounded-md px-2 py-1 text-white text-sm" },
                        ...
                        tool === 'text' && (
                            React.createElement("div", { className: "bg-zinc-900/50 p-3 rounded-lg flex flex-col gap-2 animate-fade-in" },
                                React.createElement("label", { className: "text-sm text-white font-medium", htmlFor: "text-label" }, "Texto a Añadir"),
                                 React.createElement("input", {
                                    type: "text",
                                    id: "text-label",
                                    value: textLabel,
                                ...
                                React.createElement("datalist", { id: "street-names" },
                                    streets.map(street => React.createElement("option", { key: street }, street))
                                ),
                                React.createElement("label", { className: "flex items-center gap-2 text-sm text-zinc-300 cursor-pointer" },
                                    React.createElement("input", { type: "checkbox", checked: isTextVertical, onChange: e => setIsTextVertical(e.target.checked), className: "h-4 w-4 bg-zinc-600 border-zinc-500 rounded text-blue-500 focus:ring-blue-500"}),
                                    "Texto Vertical"
                                )
                            )
                        ),
                         ...
                         selectedZone && (
                           React.createElement("div", { id: "radius-editor", className: "bg-zinc-900/50 p-3 rounded-lg flex flex-col gap-2 animate-fade-in" },
                             React.createElement("label", { htmlFor: "radius-slider", className: "text-sm text-white font-medium" }, `Editar Radio: ${selectedZone.layer.getRadius()}m`),
                             React.createElement("input", {
                               type: "range",
                               id: "radius-slider",
                               min: "10",
                 ...
                 React.createElement("div", { className: "absolute top-2 left-2 z-[1000] w-full max-w-sm" },
                    React.createElement("form", { onSubmit: handleSearchSubmit, className: "relative" },
                        React.createElement("input", {
                            type: "text",
                            value: searchQuery,
                        ...
                        React.createElement("button", { type: "submit", className: "absolute right-2 top-1/2 -translate-y-1/2 p-1 text-zinc-400 hover:text-white" },
                           React.createElement(SearchIcon, { className: "w-5 h-5"})
                        )
                    )
                ),
                 mode === 'adjusting' && (
                    React.createElement("div", { className: "croquis-lock-overlay absolute inset-0 z-[1001] bg-black/50 flex justify-center items-center backdrop-blur-sm" },
                         React.createElement("div", { className: "text-center p-8 bg-zinc-800 rounded-lg shadow-lg flex flex-col items-center gap-4" },
                            React.createElement(LockClosedIcon, { className: "w-12 h-12 text-yellow-400"}),
                            React.createElement("h3", { className: "text-2xl font-bold text-white" }, "Modo de Ajuste Final"),
                            React.createElement("p", { className: "text-zinc-300 max-w-md" }, "El croquis está validado. Mueva los elementos para el ajuste final y actualice la captura para el reporte PDF. O vuelva al modo de edición para añadir/quitar elementos.")
                        )
                    )
                )
            )

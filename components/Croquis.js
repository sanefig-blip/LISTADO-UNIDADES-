...
const Croquis = ({ isActive, onSketchCapture, onUnlockSketch, initialLayer = 'street' }) => {
     ...
     const saveStateToLocalStorage = useCallback(() => {
        ...
        try {
            const state = elements.map(el => {
                ...
                if (el.elementType === 'line') return { elementType: 'line', latlngs: el.layer.getLatLngs().map((latlng) => ({ lat: latlng.lat, lng: latlng.lng })), options: el.layer.options };
                return null;
            }).filter(Boolean);
    ...
    return (
        React.createElement("div", { ref: containerRef, className: `flex flex-col gap-4 transition-all duration-300 ${isMaximized ? 'fixed inset-0 bg-zinc-900 z-50 p-4' : 'h-[85vh]'}` },
            React.createElement("div", { className: "bg-zinc-800/60 p-3 rounded-xl flex flex-wrap items-center justify-between gap-2 shadow-sm" },
                React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                    ...
                    React.createElement(ToolButton, { toolName: "attackLine", label: "Línea Ataque", icon: React.createElement(FireIcon, { className: "w-6 h-6 text-red-500" }) }),
                    React.createElement(ToolButton, { toolName: "transferLine", label: "Línea Transf.", icon: React.createElement("div", { className: "w-6 h-6 border-2 border-dashed border-blue-500 rounded-full" }) }),
            ...
            React.createElement("div", { className: "relative w-full h-full" },
                ...
                React.createElement("div", { className: "absolute top-2 right-2 z-[1000] flex flex-col gap-2" },
                    ...
                    React.createElement("input", { ref: fileInputRef, type: "file", accept: ".kml", onChange: () => {}, style: { display: 'none' } }),
                 ...
                 React.createElement("div", { className: "absolute top-2 left-2 z-[1000] w-full max-w-sm" },
                    React.createElement("form", { onSubmit: handleSearchSubmit, className: "relative" },
                        React.createElement("input", { type: "text", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), placeholder: "Buscar dirección...", className: "w-full bg-zinc-800/80 backdrop-blur-sm border border-zinc-600 rounded-md pl-4 pr-10 py-2 text-white placeholder-zinc-400 focus:ring-blue-500 focus:border-blue-500" }),
